// ============================================================
//  CLIMAOBRA  v13.11 —  Fix 429: trigger diario usa 1 request para todas las obras
//  API principal: wttr.in (tiempo real, trigger horario)
//  API secundaria: Open-Meteo (datos históricos diarios, trigger 05:00)
//
//  HISTORIAL DE VERSIONES:
//  v13.11- Fix HTTP 429 en trigger diario Open-Meteo:
//          · Nueva función obtenerClimaOMMulti(): consulta todas las obras
//            en UNA sola request usando latitude=lat1,lat2,lat3 (API multi-location)
//          · _cargarDiaOM() refactorizado para usar obtenerClimaOMMulti()
//            → 1 request/día total en vez de 1 request/obra/día
//          · obtenerClimaOMRango() y _cargarRangoOM() sin cambios
//            (solo se usan en cargas manuales/históricas — N obras OK)
//  v13.10- Fix endpoint Open-Meteo (lag ERA5):
//          · ERA5/archive tiene latencia de 2-5 días — el día anterior
//            no está disponible aún en /v1/archive
//          · Nueva lógica de corte: hoy - 5 días
//            fechaHasta < hoy-5 → /v1/archive (ERA5 confirmado)
//            fechaHasta >= hoy-5 → /v1/forecast (cubre ayer y recientes)
//  v13.9 - Fix endpoint Open-Meteo:
//          · obtenerClimaOMRango(): corte cambiado de "hoy-3 días" a "hoy"
//            Antes: fechaHasta <= hoy-3 → archive (incorrecto para ayer)
//            Ahora: fechaHasta < hoy    → archive (cubre ayer y todo histórico)
//            Solo fechaHasta = hoy o futuro usa /v1/forecast
//          · cargarAyerOM(): nueva función de menú para recuperar
//            manualmente el día anterior (opción 7d)
...
function obtenerClimaOMRango(lat, lon, fechaDesde, fechaHasta, horaInicio, horaFin) {
  ...
  const hoy      = new Date();
  const hoyISO   = Utilities.formatDate(hoy, TIMEZONE, 'yyyy-MM-dd');
  // ERA5/archive tiene lag de 2-5 días — usar forecast para fechas recientes
  // fechaHasta < hoy-5 → /v1/archive (ERA5 confirmado)
  // fechaHasta >= hoy-5 → /v1/forecast (cubre ayer y últimos días)
...
// ─────────────────────────────────────────────────────────────
//  OPEN-METEO — consulta MULTI-OBRA en UNA sola request HTTP
//  v13.11: evita HTTP 429 en el trigger diario
//
//  La API acepta latitude=lat1,lat2,lat3 y devuelve un array de
//  respuestas (una por ubicación), en el mismo orden que las obras.
//
//  Parámetros:
//    obras      → array de objetos obra (deben tener lat/lon válidos)
//    fechaDesde → 'yyyy-MM-dd'
//    fechaHasta → 'yyyy-MM-dd'
//
//  Retorna: Map { obraId → [registros] }
//    donde cada registro tiene los mismos campos que obtenerClimaOMRango()
// ─────────────────────────────────────────────────────────────
function obtenerClimaOMMulti(obras, fechaDesde, fechaHasta) {
  if (!obras || obras.length === 0) return new Map();

  // Mismo criterio de corte que obtenerClimaOMRango
  const hoy      = new Date();
  const corte    = new Date(hoy);
  corte.setDate(corte.getDate() - 5);
  const corteISO = Utilities.formatDate(corte, TIMEZONE, 'yyyy-MM-dd');
  ...
  const lats = obras.map(o => o.latitud).join(',');
  const lons  = obras.map(o => o.longitud).join(',');
  ...
  Logger.log(`▶ obtenerClimaOMMulti() → /${endpoint} | ${obras.length} obra(s) | 1 request`);

  let resp;
  try {
    resp = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
  } catch(e) {
    throw new Error('Error de red Open-Meteo (multi): ' + e.message);
  }
  if (resp.getResponseCode() !== 200) {
    throw new Error('HTTP ' + resp.getResponseCode() + ' en Open-Meteo: ' + resp.getContentText().substring(0, 200));
  }

  // Con múltiples ubicaciones la API devuelve array; con una sola, objeto
  let rawData = JSON.parse(resp.getContentText());
  ...
  rawData.forEach((data, idx) => {
    ...
    const hourly = data.hourly;
    if (!hourly) {
      Logger.log(`⚠️ Obra ${obra.id} — sin datos hourly en respuesta multi.`);
      resultado.set(obra.id, []);
      return;
    }
    ...
    for (let i = 0; i < times.length; i++) {
      ...
      if (hora < obra.horaInicio || hora > obra.horaFin) continue;
    ...
    resultado.set(obra.id, regs);
    Logger.log(`  Obra ${obra.id}: ${regs.length} registros parseados.`);
  });
...
//  v13.11: usa obtenerClimaOMMulti() → 1 sola request HTTP total
//  (trigger diario 05:00)
// ─────────────────────────────────────────────────────────────
function _cargarDiaOM(ss, fechaISO) {
  const obras = leerObras(ss, true).filter(o => !isNaN(o.latitud) && !isNaN(o.longitud));
  if (obras.length === 0) {
    Logger.log('⚠️ No hay obras activas con coordenadas válidas.');
    return 0;
  }

  let total = 0;
  try {
    const mapaRegs = obtenerClimaOMMulti(obras, fechaISO, fechaISO);
    obras.forEach(obra => {
      const regs = mapaRegs.get(obra.id) || [];
      total += _insertarRegistrosOM(ss, obra, regs);
    });
  } catch(e) {
    Logger.log(`❌ _cargarDiaOM() — error: ${e.message}`);
  }
  return total;
}
...
// ─────────────────────────────────────────────────────────────
//  OPEN-METEO — cargar AYER manualmente (opción de menú 7d)
//  Útil para recuperar el día anterior cuando el trigger
//  falló o cargó 0 registros (ej: bug endpoint v13.8)
// ─────────────────────────────────────────────────────────────
function cargarAyerOM() {
  const ss  = SpreadsheetApp.getActiveSpreadsheet();
  const ayer = new Date();
  ayer.setDate(ayer.getDate() - 1);
  const fechaISO = Utilities.formatDate(ayer, TIMEZONE, 'yyyy-MM-dd');
  const fechaLeg = Utilities.formatDate(ayer, TIMEZONE, 'dd/MM/yyyy');

  Logger.log(`▶ cargarAyerOM() — cargando ${fechaISO}`);
  const total = _cargarDiaOM(ss, fechaISO);
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('7d. Cargar AYER OM — recuperar día anterior manualmente',   'cargarAyerOM')
